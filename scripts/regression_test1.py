#!/usr/bin/env python3
"""Regression Test 1: Single-domain, single-document (battery BATOM_001)."""
...
def main():
    print("=" * 60)
    print("REGRESSION TEST 1: Battery x BATOM_001")
    ...
    print(f"Records: {len(records_by_domain['battery'])}")
    print(f"Schema candidates: {len(candidates_by_domain['battery'])}")

    # v2 check: shared_hypernym in routing_features
    hypernym_cands = [
        c
        for c in candidates_by_domain["battery"]
        if c.routing_features.get("shared_hypernym")
    ]
    print(f"Candidates with shared_hypernym (from v1 data): {len(hypernym_cands)}")

    # Build backbone
    backbone_concepts, backbone_descriptions, curated_concepts = build_backbone(
        config=config
    )
    print(f"Backbone concepts ({len(backbone_concepts)}): {backbone_concepts}")
    ...
    accepted = sum(
        1 for d in decisions_by_domain["battery"].values() if d.admit_as_node
    )
    rejected_reasons = Counter(
        d.reject_reason
        for d in decisions_by_domain["battery"].values()
        if not d.admit_as_node
    )
    print(f"Attachment: {accepted}/{len(decisions_by_domain['battery'])} accepted")
    print(f"Rejected reasons: {dict(rejected_reasons)}")

    route_dist = Counter(d.route for d in decisions_by_domain["battery"].values())
    ...
    print(f"\n--- Workflow Edge Details ---")
    for e in sorted(
        [e for e in graph.edges if e.edge_layer == "workflow"],
        key=lambda e: (e.workflow_kind or "", e.label),
    ):
        wk_str = f" [{e.workflow_kind}]" if e.workflow_kind else ""
        adm = "shown" if e.display_admitted else "hidden"
        print(
            f"  {e.family}{wk_str}: {e.head} --{e.display_label or e.label}--> {e.tail} ({adm})"
        )

    print(f"\n--- Semantic Edge Details ---")
    for e in sorted(
        [e for e in graph.edges if e.edge_layer == "semantic"],
        key=lambda e: (e.family, e.label),
    ):
        adm = "shown" if e.display_admitted else "hidden"
        print(
            f"  {e.family}: {e.head} --{e.display_label or e.label}--> {e.tail} ({adm})"
        )

    # Save result
    result = VariantRunResult(
        variant_id=variant.variant_id,
        variant_description="rule-based regression test1",
        ...
        construction_summary={"test": "regression_test1_battery_single"},
    )
    from pipeline.artifacts import export_variant_run

    export_variant_run(
        "results/regression_v2/test1", result, False, False, True, False, False
    )
    print(f"\nResults exported to results/regression_v2/test1/")

    # Save structured results for report
    report = {
        "test": "regression_test1",
        "description": "Single-domain single-document (battery BATOM_001)",
        ...
        "attachment_stats": {
            "accepted": accepted,
            "total": len(decisions_by_domain["battery"]),
            "route_distribution": dict(route_dist),
        },
    }
    with open("results/regression_v2/test1_report.json", "w") as f:
        json.dump(report, f, indent=2, ensure_ascii=False)
    print(f"Report saved to results/regression_v2/test1_report.json")
